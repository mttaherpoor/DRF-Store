...
class InventoryFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request , model_admin):
        return [
            (self.LESS_THAN_MIN,'High'),
            (self.BETWEEN_MIN_MAX,'Medium'),
            (self.MORE_THAN_MAX,'OK'),
        ]
    
    def queryset(self, request, queryset):
        if self.value() == self.LESS_THAN_MIN:
            return queryset.filter(inventory__lt=self.MIN)
        if self.value() == self.BETWEEN_MIN_MAX:
            return queryset.filter(inventory__range=(self.MIN, self.MAX))
        if self.value() == self.MORE_THAN_MAX:
            return queryset.filter(inventory__gt=self.MAX)
